`BTC`和`ETH`这类代币都属于同质化代币，矿工挖出的第`1`枚`BTC`与第`10000`枚`BTC`并没有不同，是等价的。但世界中很多物品是不同质的，其中包括房产、古董、虚拟艺术品等，这类物品无法用同质化代币抽象。因此，[以太坊EIP721](https://eips.ethereum.org/EIPS/eip-721)提出了`ECR721`标准，来抽象非同质化的物品。

## EIP与ERC

这里有一个点需要理解，`ERC721`与`EIP721`是什么关系？

`EIP`全称`Ethereum Improvement Proposals`(以太坊改进建议)，是以太坊开发者社区提出的改进建议，是一系列以编号排定的文件，类型互联网上ETF的RFC。
