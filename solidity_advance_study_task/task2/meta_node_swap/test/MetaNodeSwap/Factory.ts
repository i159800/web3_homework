...
describe("Factory", function () {
  ...
  it("createPool", async function () {
    const { factory, publicClient } = await networkHelpers.loadFixture(deployFixture);
  ...
  it("createPool with same token", async function () {
    ...
    try {
      ...
      expect(String(error)).to.include("IDENTICAL_ADDRESSES")
    }
    
    try {
      await expect(factory.read.getPool([tokenA, tokenB, 3])).to.be.revertedWith(
        "IDENTICAL_ADDRESSES"
      );
      expect.fail("Expected getPool to revert")
    } catch(error) {
      expect(String(error)).to.include("IDENTICAL_ADDRESSES")
    }
