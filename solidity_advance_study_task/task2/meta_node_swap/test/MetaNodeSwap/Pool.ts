...
import { TickMath, encodeSqrtRatioX96 } from "@uniswapfoundation/v3-sdk";
...
describe("Pool", function () {
  ...
  it("mint and burn and collect", async function () {
    const { pool, token0, token1, sqrtPriceX96 } = await networkHelpers.loadFixture(
      deployFixture
    );
    ...
    console.log("token0 balance:", await token0.read.balanceOf([pool.address]))
    console.log("token0 Lp balance:", await token0.read.balanceOf([testLP.address]))
    console.log("token1 balance:", await token1.read.balanceOf([pool.address]))
    console.log("token1 Lp balance:", await token1.read.balanceOf([testLP.address]))
