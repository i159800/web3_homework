...
describe("PoolManager", function () {
  ...
  it("getPairs & getAllPools", async function () {
    const { manager } = await networkHelpers.loadFixture(deployFixture);
  ...
  it("require token0 < token1", async function () {
    ...
    try {
      ...
      ).to.be.revertedWith("token0 must be less than token1")
      expect.fail("Expected createPool to revert")
    } catch(error) {
      expect(String(error)).to.include("token0 must be less than token1")
    }
